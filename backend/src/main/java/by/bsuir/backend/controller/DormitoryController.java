...
public class DormitoryController extends AbstractController {
    
    private final DormitoryService service;

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public DormitoryResponseTo save(@RequestBody DormitoryRequestTo entity) {
        return service.save(entity);
    }

    @GetMapping
    @ResponseStatus(HttpStatus.OK)
    public List<DormitoryResponseTo> findAll(@RequestParam(defaultValue = "0") Integer pageNumber,
    ...
    @GetMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    public DormitoryResponseTo findById(@PathVariable Integer id) {
        return service.findById(id);
    }

    @PutMapping
    @ResponseStatus(HttpStatus.OK)
    public DormitoryResponseTo update(@RequestBody @Valid DormitoryRequestTo entity) {
